...
if 1:
    ...
    plt.savefig('figures/Dat/scatter_mat_all_feat.png', format='png', dpi=300)
    plt.show()
...
if 1:
    ...
    scatter_matrix(data[features_to_plot], figsize=(12, 8)) # Note: histograms on the main diagonal
    plt.savefig('figures/Dat/histogram_1_feature.png', format='png', dpi=300)
    plt.show()
...
if 1:
    ...
    plt.tight_layout()
    plt.savefig('figures/Dat/hist_data.png', format='png', dpi=300) # must save before show()
    plt.show()
...
data.drop(columns = ["STT", "Nhu cầu","Số lầu hiện tại","Hướng nhà","Hướng ban công","Số phòng ngủ","Số toilet","Nội thất"], inplace=True) 
...
if method == 1: 
    ...
    data["KHOẢNG GIÁ"] = pd.cut(data["Mức giá - Tỷ"],
                                    bins=[0, 20, 40, 60, 80, 100, 200, 300, np.inf],
                                    #labels=["<20 tỷ", "20-40 tỷ", "40-60 tỷ", "60-80 tỷ", "80-100 tỷ", "100-200 tỷ","200-300 tỷ",">300 tỷ"])
                                    labels=[20, 40, 60, 80, 100, 200, 300,3000]) 
    ...
    if 1:
        data["KHOẢNG GIÁ"].hist(bins=9, figsize=(5,5)); 
        plt.savefig('figures/Dat/data_hist_price.png', format='png', dpi=300)
        #plt.show();
        train_set["KHOẢNG GIÁ"].hist(bins=9, figsize=(5,5)); 
        plt.savefig('figures/Dat/train_set_hist_price.png', format='png', dpi=300)
        #plt.show()
...
num_feat_names = ['Năm' , 'Diện tích - m2', 'Mặt tiền' , 'Đường vào' , 'GPXD'] 
# =list(train_set.select_dtypes(include=[np.number]))
cat_feat_names = ['Quận' , 'Phường' , 'Đường' ,'Pháp lý'] 
...
joblib.dump(full_pipeline, r'models/full_pipeline_3.pkl')
...
def store_model(model, model_name = ""):
    ...
    joblib.dump(model,'models/' + model_name + '_GridSearch_CV' '_model_3.pkl')
def load_model(model_name):
    # Load objects into memory
    #del model
    model = joblib.load('models/' + model_name + '_GridSearch_CV' + '_model_3.pkl')
    #print(model)
    return model
