...
if 1:
    ...
    scatter_matrix(data[features_to_plot], figsize=(12, 8)) # Note: histograms on the main diagonal
    plt.savefig('figures/histogram_1_feature.png', format='png', dpi=300)
    plt.show()
...
if 1:
    ...
    plt.tight_layout()
    plt.savefig('figures/hist_data.png', format='png', dpi=300) # must save before show()
    plt.show()
...
if method == 1: 
    ...
    data["KHOẢNG GIÁ"] = pd.cut(data["Mức giá - Tỷ"],
                                    ...
                                    #labels=["<2 tỷ", "2-5 tỷ", "5-10 tỷ", "10-15 tỷ", "15-20 tỷ", ">20 tỷ"])
                                    labels=[2,5,10,15,20,100]) 
    ...
    if 1:
        data["KHOẢNG GIÁ"].hist(bins=7, figsize=(6,6)); 
        plt.savefig('figures/data_hist_price.png', format='png', dpi=300)
        #plt.show();
        train_set["KHOẢNG GIÁ"].hist(bins=7, figsize=(6,6)); 
        plt.savefig('figures/train_set_hist_price.png', format='png', dpi=300)
        #plt.show()
...
def store_model(model, model_name = ""):
    ...
    joblib.dump(model,'models/' + model_name + '_GridSearch_CV' '_model.pkl')
...
def predict_input_user(data):
    try:
        ...
        return final_model.predict(sample_prepared).tolist()[0]
